...
{
    uint32_t actual_bit_width;  // 实际位宽值（16, 18, 20, 24, 32）
    
    // 关键修复：将枚举值转换为实际位宽值
    // data_width枚举值：0=RESERVED, 1=16bit, 2=18bit, 3=20bit, 4=24bit, 5=32bit
    // 如果传入的是枚举值（0-5），转换为实际位宽；如果已经是位宽值（>=16），直接使用
    if (data_width <= 5) {
        // 枚举值，需要转换
        switch (data_width) {
            case 1: actual_bit_width = SIXTEEN_WIDTH; break;      // 16位
            case 2: actual_bit_width = EIGHTTEEN_WIDTH; break;     // 18位
            case 3: actual_bit_width = TWENTY_WIDTH; break;        // 20位
            case 4: actual_bit_width = TWENTY_FOUR_WIDTH; break;   // 24位
            case 5: actual_bit_width = THIRTY_TWO_WIDTH; break;    // 32位
            default: actual_bit_width = SIXTEEN_WIDTH; break;      // 默认16位
        }
    } else {
        // 已经是位宽值，直接使用
        actual_bit_width = data_width;
    }
    
    hal_sio_v151_crg_clock_enable(bus, false);
    hal_sio_v151_fs_div_num_set_num(bus, (actual_bit_width * number_of_channels));
    hal_sio_v151_fs_div_ratio_num_set_num(bus, (actual_bit_width * number_of_channels) / I2S_DUTY_CYCLE);
}
...
static void hal_i2s_config(sio_bus_t bus, hal_sio_mode_t mode)
{
    ...
    // 关键修复：第二个参数应该是data_width，而不是div_number
    // 这导致BCLK计算错误（使用div_number=16而不是data_width=5，导致BCLK=512kHz而不是1.024MHz）
    hal_sio_v151_i2s_drive_mode(bus, g_hal_sio_config[bus].drive_mode, g_hal_sio_config[bus].data_width,
