...
static void *i2s_audio_task(const char *arg)
{
    ...
    while (g_is_recording) {
        // 1. TX Async
        uapi_i2s_merge_write_by_dma(SIO_BUS_0, g_tx_buffer[buf_idx], FRAME_COUNT,
                                    &dma_cfg, (uintptr_t)NULL, false);

        // 2. RX Blocking
        // 注意: 传递FRAME_COUNT(2048)作为长度(Words)，因为之前2048工作过。
        int32_t ret = uapi_i2s_merge_read_by_dma(SIO_BUS_0, g_rx_buffer[buf_idx], FRAME_COUNT, 
                                   &dma_cfg, (uintptr_t)NULL, true);
        
        if(ret != 0 && g_frame_count % 100 == 0) osal_printk("[ERR] Read Ret: 0x%x\r\n", ret);
        ...
        // Send the processed audio buffer (stereo packed format)
        int ws_ret = audio_websocket_send((const uint8_t*)g_tx_buffer[next_idx], 
                                          FRAME_COUNT * sizeof(uint32_t));
        ...
        if (g_frame_count % 20 == 0) {
            uint32_t raw_packed = g_rx_buffer[buf_idx][0];
            osal_printk("[#%d] RX[0]=0x%x\r\n", g_frame_count, raw_packed); 
        }
