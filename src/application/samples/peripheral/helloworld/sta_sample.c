...
#define WIFI_SCAN_EXPECT_SSID            "testap"
...
#define WIFI_TASK_STACK_SIZE            0x8000
...
td_s32 example_sta_function(td_void)
{
    ...
    //wifi 连接成功
    if (wifi_connected) {
        PRINT("%s::STA connected and got IP successfully.\r\n", WIFI_STA_SAMPLE_LOG);
        /* Call the public entry (no args) to start the websocket task. */
        test_websocket_task();
    }
    return 0;
}
